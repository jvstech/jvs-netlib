...
jvs::Error jvs::net::create_socket_error(int ecode) noexcept
{
  ...
  if (charCount && errorText)
  {
    ...
    ::WideCharToMultiByte(CP_UTF8, 0, errorText, static_cast<int>(charCount),
      ...
      /*lpUsedDefaultChar*/ nullptr);
    auto err = jvs::make_error<SocketError>(ecode, utf8ErrMessage);
    ::LocalFree(errorText);
    errorText = nullptr;
    return err;
  }

  return jvs::make_error<SocketError>(
    ecode, "Error code " + std::to_string(ecode));
}
...
jvs::Error jvs::net::create_addrinfo_error(int ecode) noexcept
{
  ...
  return create_socket_error(wsCode);
}
